...
void SHA1Transform(u32 state[5], const u8 buffer[64])
{
    ...
    a = b = c = d = e = 0;
#ifdef SHA1HANDSOFF
    forced_memset(block, '\0', sizeof(block));
#endif
}
...
void SHA1Final(u8 digest[20], SHA1_CTX* const context)
{
    ...
    for(u32 i = 0; i < 8; ++i)
    {
        const u32 ci = context->count[(i >= 4 ? 0 : 1)];
        const u32 bs = (3 - (i & 3)) * 8;

        finalcount[i] = (u8) ((ci >> bs) & 0xFF);      /* Endian independent */
    }
    ...
    /* Wipe variables */
    forced_memset(context, '\0', sizeof(*context));
    forced_memset(&finalcount, '\0', sizeof(finalcount));
}
...
void SHA1(u8 hash_out[20], const char* const str, const size_t len)
{
    ...
    {
        SHA1Update(&ctx, (const u8*) (str + i), 1);
    }

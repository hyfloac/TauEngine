...
i64 FileReader::read1(void* const buffer) noexcept
{
    // Are we at the end of the buffer.
    if(_bufferIndex == _bufferFillCount)
    {
...
i64 FileReader::read2(void* const buffer) noexcept
{
    // Are we at the end of the buffer.
    if(_bufferIndex == _bufferFillCount)
    {
...
i64 FileReader::readCountSafe(void* const buffer, const uSys bufferSize) noexcept
{
    // Are we at the end of the buffer.
    if(_bufferIndex == _bufferFillCount)
    {
    ...
    if(_bufferIndex + bufferSize > _bufferFillCount)
    {
        ...
        (void) ::std::memcpy(reinterpret_cast<u8*>(buffer) + readSize, _buffer, bufferSize - readSize);
        _bufferIndex = bufferSize - readSize;
    }
    else
    {
        // Copy the data.
        (void) ::std::memcpy(buffer, reinterpret_cast<u8*>(_buffer) + _bufferIndex, bufferSize);
        _bufferIndex += bufferSize;
    }

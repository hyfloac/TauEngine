...
private:
    ...
    FileProps _props;
public:
    Win32File(HANDLE file, const WDynString& name, const FileProps props) noexcept
        : _file(file)
        , _name(name)
        , _props(props)
    { }

    Win32File(HANDLE file, WDynString&& name, const FileProps props) noexcept
        : _file(file)
        , _name(::std::move(name))
        , _props(props)
    { }
...
public:
    static const CPPRef<Win32FileLoader>& Instance() noexcept;
public:

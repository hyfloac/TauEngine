...
Ref<IVertexBuffer> IVertexBuffer::create(IRenderingContext& context, Type type, UsageType usage) noexcept
{
    switch(context.mode().currentMode())
    {
        ...
        case RenderingMode::Mode::OpenGL4_6:
            return Ref<IVertexBuffer>(new(std::nothrow) GLVertexBuffer(type, usage));
        default: return null;
    }
}

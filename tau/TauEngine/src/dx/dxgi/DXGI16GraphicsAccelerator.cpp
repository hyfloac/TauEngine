...
DXGI16GraphicsAccelerator::GAList DXGI16GraphicsAccelerator::graphicsAccelerators(const bool debug) noexcept
{
    ...
    for(UINT i = 0; i < adapterCount; ++i)
    {
        ...
        res = dxgiAdapter->QueryInterface(&dxgiAdapter4);
        ...
        accelerators[i] = build(dxgiAdapter4);
...
NullableRef<DXGI16GraphicsAccelerator> DXGI16GraphicsAccelerator::build(IDXGIAdapter4* const adapter) noexcept
{
    DXGI_ADAPTER_DESC3 dxgiAdapterDesc;
    const HRESULT res = adapter->GetDesc3(&dxgiAdapterDesc);
    ...
    return NullableRef<DXGI16GraphicsAccelerator>(DefaultTauAllocator::Instance(), L"", description, videoCardMemory, systemVideoMemory, sharedVideoMemory, adapter);
}

IGraphicsAccelerator::GDList DXGI16GraphicsAccelerator::graphicsDisplays() noexcept
{
    ...
    for(UINT i = 0; i < numOutputs; ++i)
    {
        ...
        res = dxgiOutput->QueryInterface(&dxgiOutput6);
        if(FAILED(res))
        { return GDList(); }

        displays[i] = DXGI16GraphicsDisplay::build(dxgiOutput6);
        dxgiOutput6->Release();
        dxgiOutput->Release();
    }

...
{
#define CHECK(VAL) do { if(FAILED(VAL)) { return nullptr; } } while(0)

    // Get the number of modes that match the specified display mode for the adapter output
    u32 numDisplayModes;
    CHECK(_dxgiAdapterOutput->GetDisplayModeList(DXGI_FORMAT_R8G8B8A8_UNORM, DXGI_ENUM_MODES_INTERLACED, &numDisplayModes, nullptr));
    ...
    {
        const auto& mode = dxgiDisplayModeList[i];
